...
export default function Header() {
  ...
  return (
    <>
      {/* Logo positioned and aligned with navbar - MOBILE RESPONSIVE */}
      <div className="fixed top-4 sm:top-6 left-4 sm:left-6 z-50">
        <a href="/" className="group flex items-center space-x-2 sm:space-x-3">
          {/* Axentrixx Logo - smaller on mobile */}
          <div className="relative rounded-full group-hover:bg-white/10 transition-all duration-200 flex items-center justify-center" style={{ width: '45px', height: '45px' }}>
            <img
              src="/logo.png"
              alt="Axentrixx"
          ...
          {/* Axentrixx Text - HIDDEN on mobile, shown on sm+ */}
          <span
            className="hidden sm:inline-block text-[20px] sm:text-[26px] lg:text-[30px] font-bold tracking-[0.15em] text-[#D1D5DB] dark:text-[#D1D5DB] transition-colors duration-200 group-hover:text-[#2563EB]"
      ...
      <header className="fixed top-4 sm:top-6 left-1/2 transform -translate-x-1/2 z-40 px-2">
        <nav className="bg-white/10 dark:bg-white/10 backdrop-blur-xl rounded-full px-4 sm:px-8 py-2 sm:py-3 border-2 border-[#60A5FA]/30 dark:border-[#60A5FA]/30">
          <div className="flex items-center justify-center space-x-4 sm:space-x-8">
            ...
            {/* Mobile Menu Button - smaller icon on mobile */}
            <button
      ...
      {/* Mobile Menu Dropdown - better mobile positioning */}
      {isMobileMenuOpen && (
        <div className="fixed top-16 sm:top-20 left-1/2 transform -translate-x-1/2 z-30 xl:hidden w-[90vw] max-w-sm">
